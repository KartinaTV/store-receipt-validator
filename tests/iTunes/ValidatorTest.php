<?php

use PHPUnit\Framework\TestCase;
use ReceiptValidator\iTunes\Validator as iTunesValidator;
use ReceiptValidator\RunTimeException;

/**
 * @group library
 *
 * @internal
 * @coversNothing
 */
final class iTunesValidatorTest extends TestCase
{
    private $testInvaildReceiptData = '';

    /**
     * @var iTunesValidator
     */
    private $validator;

    protected function setUp()
    {
        parent::setUp();

        $this->validator = new iTunesValidator(iTunesValidator::ENDPOINT_SANDBOX);
    }

    public function testInvalidOptionsToConstructor()
    {
        $this->expectException(RunTimeException::class);

        new iTunesValidator('in-valid');
    }

    public function testSetEndpoint()
    {
        $this->validator->setEndpoint(iTunesValidator::ENDPOINT_PRODUCTION);

        static::assertEquals(iTunesValidator::ENDPOINT_PRODUCTION, $this->validator->getEndpoint());
    }

    public function testSetReceiptData()
    {
        $this->validator->setPurchaseToken('test-data');
        $reflection = new ReflectionClass(iTunesValidator::class);
        $method = $reflection->getMethod('getPurchaseToken');
        $method->setAccessible(true);
        $result = $method->invoke($this->validator);
        static::assertEquals('test-data', $result);
    }

    public function testSetSharedSecret()
    {
        $this->validator->setSharedSecret('test-shared-secret');

        static::assertEquals('test-shared-secret', $this->validator->getSharedSecret());
    }
}
